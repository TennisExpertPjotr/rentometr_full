...
def train_lg_model():
    ...
    # Если модель линейной регрессии ещё не обучена — обучаем её
    ...
    # Формируем бинарную целевую переменную: 1 — цена выше на >= 10%
    threshold = 1.1
    ...
    # Создаём pipeline с масштабированием и логистической регрессией
    logistic_model = make_pipeline(
        StandardScaler(),
        LogisticRegression(max_iter=500)  # Можно указать solver='saga' для L1
    )
...
'''
Предсказывает, является ли цена завышенной.
...
def predict_with_lg(vector, original_price):
    ...
    predicted_price = lm.linear_model.predict(vector)[0].item()
    residual = original_price - predicted_price
    print(residual)
    ...
    # Предсказание вероятности и класса
    prediction = logistic_model.predict(vector_with_price)[0]
    probability = logistic_model.predict_proba(vector_with_price)[0][1]
    print(probability)

    if prediction:
        result = f"Цена выше ожидаемой на {residual:.0f}₽ (вероятность завышения: {probability:.2f})"
    else:
        result = f"Цена в пределах нормы (вероятность завышения: {probability:.2f})"
